...
import { editorDialogState, editorDialogActions } from "@/lib/editor-store";
...
export default function RichTextEditor({
 ...
 // Use Valtio state for dialog management
 const editorState = useSnapshot(editorDialogState);

 // Local state for non-dialog related functionality
  ...
  onUpdate: ({ editor }) => {
   ...
   if (currentLine && currentLine.endsWith("/")) {
    ...
    editorDialogActions.openCommandMenu({
     position: { top: coords.top + 30, left: coords.left },
    });
   } else if (
 ...
 useEffect(() => {
  ...
  // Valtio state subscriptions - no need for custom event listeners anymore
  // The state changes are automatically tracked by useSnapshot
 ...
 const handleCommand = (command: string) => {
  ...
  switch (command) {
   ...
   case "codeBlock":
    ...
    break;
   case "mathLatex":
    editorDialogActions.openLatexDialog({ latex: "", inline: false });
    break;
   case "divider":
   ...
   case "table-large":
    ...
    break;
   case "citation":
    editorDialogActions.openCitationDialog({ id: "", text: "", url: "" });
    break;
   case "footnote":
    editorDialogActions.openFootnoteDialog({ id: "", text: "" });
    break;
   case "bibliography":
 ...
 const handleMathLatexUpdate = (latex: string, inline?: boolean) => {
  if (editor && editorState.latexDialog.data) {
   const { pos, nodeType } = editorState.latexDialog.data;
 ...
 const handleCitationUpdate = (data: {
  ...
  if (editor && editorState.citationDialog.data) {
   const { pos, nodeType } = editorState.citationDialog.data;
   if (typeof pos === "number") {
    editor
     .chain()
     .focus()
 ...
 const handleFootnoteUpdate = (data: { id: string; text: string }) => {
  if (editor && editorState.footnoteDialog.data) {
   const { pos, nodeType } = editorState.footnoteDialog.data;
   if (typeof pos === "number") {
    editor
     .chain()
     .focus()
 ...
 return (
  <div className="relative w-full h-full">
   ...
   <div className="relative bg-white">
    ...
    {editorState.commandMenu.open && (
     <CommandMenu
      position={editorState.commandMenu.data?.position || { top: 0, left: 0 }}
    ...
    {editorState.imageDialog.open && (
     <ImageDialog
      ...
      onInsert={
       editorState.imageDialog.data ? handleImageUpdate : handleImageInsert
      }
      {...(editorState.imageDialog.data && { onDelete: handleImageDelete })}
    ...
    {editorState.latexDialog.open && (
     <ToolbarLaTeXDialog
      open={editorState.latexDialog.open}
      onClose={() => {
       editorDialogActions.closeLatexDialog();
      }}
      onInsert={
       editorState.latexDialog.data ? handleMathLatexUpdate : handleLaTeXInsert
      }
      {...(editorState.latexDialog.data && {
       initialData: {
        latex: editorState.latexDialog.data.latex,
        inline: editorState.latexDialog.data.inline,
       },
      })}
     />
    )}
    {editorState.citationDialog.open && (
     <ToolbarCitationDialog
      open={editorState.citationDialog.open}
      onClose={() => {
       editorDialogActions.closeCitationDialog();
      }}
      onInsert={
       editorState.citationDialog.data
        ? handleCitationUpdate
        : handleCitationInsert
      }
      {...(editorState.citationDialog.data && {
       initialData: {
        id: editorState.citationDialog.data.id,
        text: editorState.citationDialog.data.text,
        url: editorState.citationDialog.data.url,
       },
      })}
     />
    )}
    {editorState.footnoteDialog.open && (
     <ToolbarFootnoteDialog
      open={editorState.footnoteDialog.open}
      onClose={() => {
       editorDialogActions.closeFootnoteDialog();
      }}
      onInsert={
       editorState.footnoteDialog.data
        ? handleFootnoteUpdate
        : handleFootnoteInsert
      }
      {...(editorState.footnoteDialog.data && {
       initialData: {
        id: editorState.footnoteDialog.data.id,
        text: editorState.footnoteDialog.data.text,
       },
      })}
     />
    )}
