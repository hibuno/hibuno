...
export function createTableHTML(rows: number, cols: number, header: boolean = true): string {
...
export function createTableWithData(data: string[][], headerRow: boolean = true): string {
...
export const advancedTableTemplates: Record<string, { name: string; template: TableTemplate; data?: string[][] }> = {
	'pricing-table': {
		name: 'Pricing Table',
...
export function getTableDimensions(tableHtml: string): { rows: number; cols: number } {
...
export function addTableRow(tableHtml: string, insertAfter?: number): string {
	...
	if (insertAfter !== undefined && insertAfter < rows.length && rows[insertAfter]) {
...
export function getTableStats(tableHtml: string): {
	...
	const table = tempDiv.querySelector('table');
	if (!table) return { rows: 0, cols: 0, cells: 0, mergedCells: 0, emptyCells: 0 };
...
export function validateTable(tableHtml: string): {
	...
	rows.forEach((row, index) => {
		...
		if (cols !== firstRowCols) {
			...
			warnings.push(`Row ${index + 1} has ${cols} columns, expected ${firstRowCols}`);
		}
	});
...
export function addTableColumn(tableHtml: string, insertAfter?: number): string {
	const tempDiv = document.createElement('div');
	tempDiv.innerHTML = tableHtml;
	...
	rows.forEach((row, rowIndex) => {
		const cells = row.querySelectorAll('td, th');
		const newCell = document.createElement(rowIndex === 0 && row.closest('thead') ? 'th' : 'td');
...
export function mergeTableCells(tableHtml: string, startRow: number, startCol: number, endRow: number, endCol: number): string {
	const tempDiv = document.createElement('div');
	tempDiv.innerHTML = tableHtml;
	...
	const rows = table.querySelectorAll('tr');
	const firstRow = rows[0];
	if (!firstRow || startRow >= rows.length || startCol >= firstRow.querySelectorAll('td, th').length) {
		return tableHtml;
	}
