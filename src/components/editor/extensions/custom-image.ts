...
export const CustomImage = Image.extend({
		...
		return {
			...
			width: {
				...
				parseHTML: (element: Element) => {
					...
					return htmlElement.getAttribute('width') || htmlElement.style.width || '100%';
				},
			...
			alignment: {
				default: 'center',
				parseHTML: (element: Element) => element.getAttribute('data-alignment') || 'center',
				renderHTML: (attributes: any) => {
					return {
	...
	renderHTML({ HTMLAttributes }: { HTMLAttributes: any }) {
		...
		return [
			'div',
			{ class: `image-wrapper my-4 ${alignment === 'center' ? 'text-center' : ''}` },
			[
				'img',
				mergeAttributes(this.options.HTMLAttributes, HTMLAttributes, imgAttributes),
			],
			caption
	...
	addNodeView() {
		return ({ node, getPos, editor }: { node: any; getPos: () => number; editor: any }) => {
			...
			dom.className = `image-wrapper my-4 ${node.attrs.alignment === 'center' ? 'text-center' : ''}`;
